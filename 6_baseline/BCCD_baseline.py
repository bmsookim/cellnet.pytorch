...
file_list = os.listdir('/home/bumsoo/Data/_test/BCCD_FULL/')
...
for f in file_list:
    img_dir = ('/home/bumsoo/Data/_test/BCCD_FULL/%s' %(f))
    ...
    ret, dilation = cv2.threshold(dilation, dilation_value, 255, cv2.THRESH_BINARY)
    I_3 = dilation==I_2
    ...
    for cnt in contours:
        ...
        if area > contour_value**2 and area < 300**2:
    ...
    for i in range(len(area_array)):
        for j in range(i+1,len(area_array)):
            ...
            if (distance < distance_value) and (union_area < max_area):
    ...
    check_and_mkdir('./results/BCCD/%s' %str(threshold_value))
    csvfile = open(os.path.join('./results', 'BCCD', str(threshold_value), os.path.splitext(f)[0]+'.csv'), 'w', encoding='utf-8', newline='')
    ...
    cv2.imwrite(os.path.join('./results/', 'BCCD', str(threshold_value), os.path.splitext(f)[0]+".png"), final_img)
    ...
    #cv2.imwrite(os.path.join('./results/', 'BCCD', str(threshold_value), os.path.splitext(f)[0]+"_"+'dual_img.png'), dual_img)
