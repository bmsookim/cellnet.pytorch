...
#for thresh in [170, 175, 180]:
search_lst = ['Lymphocyte', 'typical', 'egmented', 'Band', 'mudge', 'sinophil', 'asophil', 'onocyte']
        ...
        with open('./results/baseline/%d/TEST%d/TEST%d.csv' %(thresh,i,i)) as pred_csv:
            with open('/home/bumsoo/Data/test/Test_tight/TEST%d/TEST%d.csv' %(i,i)) as answ_csv:
                ...
                for row in pred_reader:
                    #lst_A.append(map(int, row[1:]))
                    lst_A.append(row)
                    pred = row[0]
                    #lst_A.append(map(int, row))

                for row in answ_reader:
                    #lst_B.append(map(int, row[1:]))
                    lst_B.append(row)
                    label = row[0]
                ...
                for comp_A in lst_A:
                    ...
                    for comp_B in lst_B:
                        ...
                        if(IOU != 0.0):
                            if (search == 'Lymphocyte'):
                                if (label == 'Lymphocyte') and (pred == 'WBC_Lymphocyte'):
                                    TP += 1
                                    tot_TP += 1
                                elif (pred == 'WBC_Lymphocyte' and label != 'Lymphocyte'):
                                    FP += 1
                                    tot_FP += 1
                            elif (search in label) and (search in pred) and IOU >= 0.5):
                                TP += 1
                                tot_TP += 1
                            elif(search in pred):
                                FP += 1
...
print(tot_TP)
print(tot_FP)
print(float(tot_TP) / (tot_TP + tot_FP))
print("======================================")
